...
with sqlite3.connect(power_db) as c:
    ...
    def plotter(data_entries, title):
        ...
        avg_power_y = []
        for entry in range(len(power_y)-5):
            sample = power_y[entry:entry + 5]
            avg_power_y.append(sum(sample)/len(sample))
        avg_current_y = []
        for entry in range(len(current_y)-5):
            sample = current_y[entry:entry + 5]
        ...
        ax2.plot(np.array(time_x[:-5]), np.array(avg_current_y))
        ax2.set_ylim([0, 1500])
        ax2.set(xlabel = 'Time (s)', ylabel = "Current (mA)")
        ax3.plot(np.array(time_x), np.array(power_y))
        ax3.plot(np.array(time_x[:-5]), np.array(avg_power_y))
        ax3.set_ylim([0, 5])
